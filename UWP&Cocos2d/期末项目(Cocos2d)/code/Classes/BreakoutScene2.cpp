...
void Breakout2::setPhysicsWorld(PhysicsWorld* world) { m_world = world; }

Scene* Breakout2::createScene() {
	auto scene = Scene::createWithPhysics();
	//scene->getPhysicsWorld()->setDebugDrawMask(PhysicsWorld::DEBUGDRAW_ALL);
	scene->getPhysicsWorld()->setGravity(Point(0, 0));

	auto layer = Breakout2::create(scene->getPhysicsWorld());
	scene->addChild(layer);
	return scene;
}

bool Breakout2::init(PhysicsWorld* world) {
	if (!Layer::init()) {
		return false;
	}
	...
	this->schedule(schedule_selector(Breakout2::update), 1);
	return true;
}

Breakout2 * Breakout2::create(PhysicsWorld * world) {
	Breakout2* pRet = new(std::nothrow) Breakout2();
...
void Breakout2::playBgm() {
	SimpleAudioEngine::getInstance()->playBackgroundMusic("music/bgm.mp3", true);
}

void Breakout2::playMeet() {
	SimpleAudioEngine::getInstance()->playEffect("music/meet_stone.wav", false, 1.0f, 1.0f, 1.0f);
}

void Breakout2::addBackground() {
	auto bgsprite = Sprite::create("black_hole_bg1.jpg");
...
void Breakout2::addContactListener() {
	auto touchListener = EventListenerPhysicsContact::create();
	touchListener->onContactBegin = CC_CALLBACK_1(Breakout2::onConcactBegan, this);
	_eventDispatcher->addEventListenerWithFixedPriority(touchListener, 1);
}

void Breakout2::addTouchListener() {
	auto touchListener = EventListenerTouchOneByOne::create();
	touchListener->onTouchBegan = CC_CALLBACK_2(Breakout2::onTouchBegan, this);
	touchListener->onTouchMoved = CC_CALLBACK_2(Breakout2::onTouchMoved, this);
	touchListener->onTouchEnded = CC_CALLBACK_2(Breakout2::onTouchEnded, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(touchListener, this);
}

//1、2、3依次对应不同的特效
void Breakout2::addKeyboardListener() {
	auto keboardListener = EventListenerKeyboard::create();
	keboardListener->onKeyPressed = CC_CALLBACK_2(Breakout2::onKeyPressed, this);
	keboardListener->onKeyReleased = CC_CALLBACK_2(Breakout2::onKeyReleased, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(keboardListener, this);
}

bool Breakout2::onTouchBegan(Touch *touch, Event *unused_event) {
...
void Breakout2::onTouchMoved(Touch *touch, Event *unused_event) {
}

void Breakout2::onTouchEnded(Touch *touch, Event *unused_event) {
}

//移除stone并播放效果
void Breakout2::removeFromEnemys(int tag, bool flag2) {
	playMeet();
...
//特效标签 得到奖励加1 使用一次减1
void Breakout2::addRewardLabel(int tag) {
...
bool Breakout2::onConcactBegan(PhysicsContact& contact) {
...
void Breakout2::stopAc() {
	this->unscheduleAllCallbacks();
	_eventDispatcher->removeAllEventListeners();
	SimpleAudioEngine::getInstance()->pauseBackgroundMusic();
	gameOver();
	/*cocos2d::Label* label;
	label = Label::createWithTTF("Game Over!", "fonts/arial.ttf", 60);
	label->setPosition(visibleSize.width / 2, visibleSize.height - 80);
	addChild(label, 4);
	Director::getInstance()->pause();*/
}

void Breakout2::update(float f) {
	holdtime++;
	char string1[15];
	...
	if (flag == true) {
		def->setVisible(true);
		this->schedule(schedule_selector(Breakout2::update2), 0.01);
	}
	else {
		def->setVisible(false);
	}
...
void Breakout2::update2(float f) {
	Vec2 pos;
	pos = player->getPosition();
	def->setPosition(pos);
}
...
void Breakout2::addReward(int type, Point p) {
	char path[100];
	int tag;
	switch (type)
	{
...
void Breakout2::addEnemy(int type, Point p) {
	char path[100];
	int tag;
	switch (type)
	{
...
void Breakout2::onKeyPressed(EventKeyboard::KeyCode code, Event* event) {
	switch (code)
	{
...
void Breakout2::onKeyReleased(EventKeyboard::KeyCode code, Event* event) {
	switch (code)
	{
	case cocos2d::EventKeyboard::KeyCode::KEY_LEFT_ARROW:
	case cocos2d::EventKeyboard::KeyCode::KEY_A:
...
void Breakout2::gameOver() {
	...
	auto returnitem = MenuItemLabel::create(Label::createWithTTF("Return to Menue", "fonts/ALTEA.ttf", 20),
		CC_CALLBACK_1(Breakout2::returnTOmenue, this));
	...
	auto submititem = MenuItemLabel::create(Label::createWithTTF("Submit", "fonts/ALTEA.ttf", 20),
		CC_CALLBACK_1(Breakout2::submit_time, this));
	...
	auto restartitem = MenuItemLabel::create(Label::createWithTTF("Restart", "fonts/ALTEA.ttf", 20),
		CC_CALLBACK_1(Breakout2::restart, this));
...
void Breakout2::returnTOmenue(cocos2d::Ref* pSender)
{
...
void Breakout2::submit_time(cocos2d::Ref* pSender)
{

}

void Breakout2::restart(cocos2d::Ref* pSender) 
{
	auto scene = Breakout2::createScene();
	Director::getInstance()->replaceScene(
	TransitionFade::create(0.5, scene, Color3B(0, 255, 255)));
}
...
void Breakout::removeFromEnemys(int tag, bool flag2) {
	...
	if (flag2) {
		...
		else if (tag == 12) HPbarPercentage = 0;  /*碰到骷髅或者骷髅，直接GameOver*/
		else if (tag == 13) HPbarPercentage -= 50; /*碰到保护球，血减50*/
...
void Breakout::addJoint() {
	...
	center->getPhysicsBody()->setDynamic(false); // 固定中心
	center->getPhysicsBody()->setTag(12); // player碰到即输
	...
	circle->getPhysicsBody()->setVelocity(Vec2(0, 600)); // 高速旋转
	addChild(circle, 1);
